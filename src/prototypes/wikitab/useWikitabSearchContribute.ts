...
import {
  createWikitabSearchContributeFeed,
  type WikitabSearchContributeFeed,
  type WikitabSearchContributeItem,
  type WikitabSearchTopTitle,
} from './data/fetchWikitabSearchContribute'

export type ContributeSlot =
  | { kind: 'loading'; id: string }
  | { kind: 'resolved'; item: WikitabSearchContributeItem }

const INITIAL_LOADING_SLOTS = 3

export function useWikitabSearchContribute(
  searchQuery: Ref<string>,
  knownTitles: Ref<WikitabSearchTopTitle[]>,
  articlesLoading: Ref<boolean>,
  enabled: Ref<boolean>,
) {
  const slots = ref<ContributeSlot[]>([])
  const loading = ref(false)
  const fillingInitial = ref(false)
  const loadingMore = ref(false)
  const hasMore = ref(false)

  let feed: WikitabSearchContributeFeed | null = null
  ...
  async function loadInitial(query: string): Promise<void> {
    ...
    try {
      ...
      feed = await createWikitabSearchContributeFeed(query, {
        signal,
